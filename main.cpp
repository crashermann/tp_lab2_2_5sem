#include "helper.h" #include <iostream> #include <string> #include <Windows.h>
...
std::string task(const string& s, const string& word) {
    ...
    return !ans.empty() ? ans : "Файл должен иметь формат .txt или .dat\n";
}

string readFile(ifstream& input) {
    string s = "";
    string tmpS;
    while (getline(input, tmpS)) s.append(tmpS).append("\n");
    return s;
}
